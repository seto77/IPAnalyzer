...
//           if (formMain.FilePath + formMain.FileName != textBoxPrimaryFileName.Text)
 ...
 //       if (formMain.FilePath + formMain.FileName != textBoxSecondaryFileName.Text)
    ...
    private void backgroundWorkerRefine_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
    {
        ...
        sw.Stop();
        resetHorizontalMode();
        formMain.toolStripStatusLabel.Text = Crystallography.Localization.Loc(en: "Calculating Time (Find Parameter):  ", ja: "計算時間 (パラメータ探索):  ", de: "Berechnungszeit (Parameter finden):  ", fr: "Temps de calcul (Rechercher le paramètre) :  ", es: "Tiempo de cálculo (Buscar parámetro):  ", pt: "Tempo de cálculo (Encontrar parâmetro):  ", it: "Tempo di calcolo (Trova parametro):  ", ru: "Время вычисления (Поиск параметра):  ", zhHans: "计算时间（查找参数）：  ", zhHant: "計算時間 (尋找參數):  ", ko: "계산 시간 (Find Parameter):  ") + (sw.ElapsedMilliseconds).ToString() + "ms";
    ...
    private void RefineParameters()
    {
        ...
        toolStripStatusLabel1.Text = string.Format(Crystallography.Localization.Loc(en: "Completed !  Elapsed time: {0}sec.", ja: "完了 !  経過時間: {0}秒", de: "Fertig !  Verstrichene Zeit: {0}Sek.", fr: "Terminé !  Temps écoulé : {0} s.", es: "¡Completado!  Tiempo transcurrido: {0}seg.", pt: "Concluído !  Tempo decorrido: {0}seg.", it: "Completato !  Tempo trascorso: {0}sec.", ru: "Завершено !  Затраченное время: {0}с.", zhHans: "已完成！  耗时：{0}秒。", zhHant: "已完成！  經過時間: {0}秒。", ko: "완료 !  경과 시간: {0}sec."), (sw.ElapsedMilliseconds / 1000.0).ToString("f2")); // 260626Cl Loc化
    }
    ...
    private void DrawTiltCorrectionMain(List<List<PointD>> EllipseCenter, List<PointD> DirectSpots, bool IsPrimary)
    {
        ...
        using (var g = Graphics.FromImage(bmp))
        {
            ...
            g.DrawString(IsPrimary ? Crystallography.Localization.Loc(en: "Primary", ja: "プライマリ", de: "Primär", fr: "Primaire", es: "Primario", pt: "Primário", it: "Primario", ru: "Основной", zhHans: "主", zhHant: "主要", ko: "1차") : Crystallography.Localization.Loc(en: "Secondary", ja: "セカンダリ", de: "Sekundär", fr: "Secondaire", es: "Secundario", pt: "Secundário", it: "Secondario", ru: "Вторичный", zhHans: "次", zhHant: "次要", ko: "2차"), TahomaFont8, Brushes.Black, 0, 0); // 260626Cl Loc化
